...
const ACCOUNTED = {
  ...
  "web/GradientStopBar.svelte": {
    keys: ["ArrowDown", "ArrowLeft", "ArrowRight", "ArrowUp"],
    coverage: "OS: a stop bead is a role=slider, and arrow-nudging one is the platform ARIA convention web/AngleField's dial and lib/ColorPicker's three strips already ride — the SAME classification, because it is the same affordance (a positional handle over a declared range) with the same keys and no app-invented vocabulary of its own. Nothing else here is a keyboard gesture: moving, adding, selecting and purging are pointer or <button> paths. The bead CLAIMS the plain keyspace while focused (src/lib/fieldKeys.js fieldOwnsKeydown → stopPropagation) for the dial's reason, which bites harder here: a bead is a real <button>, so App.svelte's isTypingTarget() reports false and Backspace would have deleted the widget whose gradient was being edited. Cmd/Ctrl/Alt combos and Tab/Escape/Enter keep bubbling, so app undo and the enclosing panel's dismissals are untouched.",
  },
  ...
  "web/GalleryPopup.svelte": {
    keys: ["Enter", "Escape"],
    coverage: "contextual:gallery: Enter 'Search now' (skips the debounce) lights while the search box has focus, Escape 'Close' closes the whole popup from EITHER the search box or the tile grid (data-hint-popover='gallery' on the dialog root, core popover, POPOVER_HINTS.gallery). ITS OWN kind rather than a forced fit into 'search': that kind's Enter chip reads 'Choose', but nothing here is chosen by keyboard — a cell is picked by CLICKING a tile (pick(cell)), never by Enter, so reusing 'search' would announce a key that does not do what its label says. Both handlers stopPropagation so the Inspector's own keys do not also fire behind an open popup.",
  },
