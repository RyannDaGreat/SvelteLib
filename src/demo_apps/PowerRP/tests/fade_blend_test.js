...
test("THE MODE ROSTER IS REGISTERED, with the help text the Inspector renders", () => {
  ...
  assert.deepEqual(interpModeIds(), ["tween", "step", "fade", "blend", "expTween", "morph", "blurFade", "manim", "grow"]);
  ...
  // The NAMED visibility modes (WORKSTREAMS FF2/JJ, and `grow` from BS). "Manim"
  // is the user's own name for it, not a description — see its registration.
  ...
  for (const id of ["fade", "blend", "morph", "blurFade", "manim", "grow"]) assert.ok(interpMode(id).help.length > 20, `${id} needs real help text`);
});
