...
import { aimOverlay, bindRefusal, centreOnAnchorExpr, centreOnAnchorPairs, BENTO_BIND_HANDLER } from "../web/bentoBind.js";
import { activations, canvasModes, handlerFor, migrationPlan } from "../web/widget_handlers.js";
...
test("REFUSED: a GROUP MEMBER — measured off by the group's influence, so never written", () => {
  ...
  assert.ok(refusal && /member of a group/.test(refusal), refusal);
...
test("aimOverlay draws the cell as a CLOSED WORLD ring (correct under rotation)", () => {
  const CORNERS = bentoPlugin.cellGrid.corners;
  assert.deepEqual(aimOverlay(null, null, CORNERS), { chains: [], rects: [], dots: [] });
  const cell = { x: 0, y: 0, w: 10, h: 20 };
  assert.deepEqual(bentoCellCorners(cell), [[0, 0], [10, 0], [10, 20], [0, 20]]);
  const quarter = aimOverlay({ x: 0, y: 0, rotation: Math.PI / 2, scale: 1 }, cell, CORNERS);
  ...
  // one dot that cannot be confused with the bento's own resize handles.
  const flat = aimOverlay({ x: 0, y: 0, rotation: 0, scale: 1 }, cell, CORNERS);
  assert.deepEqual(flat.dots[0], { x: 5, y: 10, hot: true });
  assert.equal(flat.dots.length, 5);
  assert.ok(flat.dots.every((d) => d.hot), "every aim dot is HOT (the guide accent)");
});
