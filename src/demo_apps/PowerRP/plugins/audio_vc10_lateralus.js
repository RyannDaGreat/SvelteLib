/**
 * VCV LATERALUS — the `vcvLateralus` engine module as a PowerRP node widget.
 *
 * FILTER FAMILY (filter family): it shapes a signal it is given.
 *
 * FIDELITY TIER: VULT DSP + BEHAVIOUR ASSEMBLY. What that means for this node — which source it was
 * read from, at which commit, and which of its parts are transcribed rather
 * than modelled — is `VCV_LATERALUS_SPEC`'s `derivation` and its `help`.
 *
 ...
 * and that data is `VCV_LATERALUS_SPEC` in core/audio_specs_vc10.js.
 *
...
import { VCV_LATERALUS_SPEC } from "../core/audio_specs_vc10.js";

export const audioVcvLateralusPlugin = audioNodePlugin(VCV_LATERALUS_SPEC);
