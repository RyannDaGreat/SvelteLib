/**
 * VCV VESSEK — the `vcvVessek` engine module as a PowerRP node widget.
 *
 * SOURCE FAMILY (warm amber header): it generates signal from nothing.
 *
 * FIDELITY TIER: BEHAVIOUR MODEL. What that means for this node — which source it was
 * read from, at which commit, and which of its parts are transcribed rather
 * than modelled — is `VCV_VESSEK_SPEC`'s `derivation` and its `help`.
 *
 ...
 * and that data is `VCV_VESSEK_SPEC` in core/audio_specs_vc10.js.
 *
...
import { VCV_VESSEK_SPEC } from "../core/audio_specs_vc10.js";

export const audioVcvVessekPlugin = audioNodePlugin(VCV_VESSEK_SPEC);
