/**
 * ENVELOPE — the `adsr` engine module as a PowerRP node widget.
 *
 * MODULATION family (muted blue header): it drives other modules rather than being heard.
 *
 * Attack/Decay/Sustain/Release with proper retrigger. Wire a gate in and its output into a VCA's `gain` — that is how a sound gets a shape instead of just switching on.
 *
 ...
 * ADSR_SPEC in core/audio_specs.js — its ports, its knobs and their ranges,
...
import { ADSR_SPEC } from "../core/audio_specs.js";

export const audioAdsrPlugin = audioNodePlugin(ADSR_SPEC);
