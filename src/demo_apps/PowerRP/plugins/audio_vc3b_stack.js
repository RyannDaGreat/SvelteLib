/**
 * Bogaudio Stack — the ported `Bogaudio-Stack` as a PowerRP node widget.
 *
 * MODULATION family (muted blue header): it drives other nodes rather than being heard.
 *
 * Bogaudio's V/oct transposer. Its `thru` output becomes a CV SOURCE when nothing is patched in, which is how two of them chain.
 *
 * ── WHY THIS FILE IS TWO LINES ──────────────────────────────────────────────
 * Every audio node has the SAME shape and that shape lives ONCE in
 * core/audio_nodes.js. What differs per module is DATA — here VCV_STACK_SPEC in
...
import { VCV_STACK_SPEC } from "../core/audio_specs_vc3b.js";

export const audioVcvStackPlugin = audioNodePlugin(VCV_STACK_SPEC);
