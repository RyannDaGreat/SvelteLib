...
import { trailPlugin } from "./trail.js"; // SIMULATED-state streamer: follows an anchored point for N seconds, samples stored in the simulation history table (core/trail_history.js)
...
import { irisBladesPlugin } from "./iris_blades.js"; // the same iris diaphragm drawn as its BLADE ASSEMBLY: N overlapping stroked plates, each buried under the next, so the opening is what their overlap leaves rather than a shape that was drawn (shared geometry and rows with aperture via core/optics.js)
...
// THE NODE-FLOW TRIO (core/nodeflow.js): the first widgets that declare PORTS, and
// the proof that a typed value flows across a WIRE — number source → math → display.
// Deliberately NON-AUDIO: the node system is general (user ruling: "we'll also be
// doing nodes for materials and shapes"), so its first client must not be the one
// that makes it audio-shaped.
...
// THE VISUAL NODE (core/visual_node.js): a node that does nothing — customizable
...
// why the roster is a barrel rather than 23 lines in this import block.
