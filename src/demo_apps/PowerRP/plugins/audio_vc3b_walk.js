/**
 * Bogaudio Walk — the ported `Bogaudio-Walk` as a PowerRP node widget.
 *
 * MODULATION family (muted blue header): it drives other nodes rather than being heard.
 *
 * Bogaudio's smooth random walker — a leaky integrator over reflected noise, not smoothed noise. It wanders and stays where it wanders.
 *
 * ── WHY THIS FILE IS TWO LINES ──────────────────────────────────────────────
 * Every audio node has the SAME shape and that shape lives ONCE in
 * core/audio_nodes.js. What differs per module is DATA — here VCV_WALK_SPEC in
...
import { VCV_WALK_SPEC } from "../core/audio_specs_vc3b.js";

export const audioVcvWalkPlugin = audioNodePlugin(VCV_WALK_SPEC);
