/**
 * AX DECAY ENVELOPE — the `axEnvDecay` engine module as a PowerRP node widget.
 *
 * MODULATION family (muted blue header): it shapes or routes other nodes rather than being heard on its own.
 *
 * `env/d`: a trigger snaps it to full scale and it falls away exponentially. Every percussive voice in an Axoloti patch is one of these driving a VCA.
 *
 ...
 * and that data is AX_ENV_DECAY_SPEC in core/audio_specs_ax4.js.
 *
...
import { AX_ENV_DECAY_SPEC } from "../core/audio_specs_ax4.js";

export const audioAxEnvDecayPlugin = audioNodePlugin(AX_ENV_DECAY_SPEC);
