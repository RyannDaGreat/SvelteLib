/**
 * Axoloti K-Smoother — the ported `kfilter/lowpass` + `tiar/kfilter/LPRiseDecay` as a PowerRP node widget.
 *
 * FILTER family (cool teal header): it shapes a spectrum that already exists.
 *
 * A CONTROL-RATE one-pole with separate rise and fall rates — what you put in front of a modulation input.
 *
 * ── WHY THIS FILE IS TWO LINES ──────────────────────────────────────────────
 * Every audio node has the SAME shape and that shape lives ONCE in
 * core/audio_nodes.js. What differs per module is DATA — here AX_KFILTER_LOWPASS_SPEC in
...
import { AX_KFILTER_LOWPASS_SPEC } from "../core/audio_specs_ax3.js";

export const audioAx3KFilterLowpassPlugin = audioNodePlugin(AX_KFILTER_LOWPASS_SPEC);
