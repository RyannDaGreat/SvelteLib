/**
 * SUPERSAW — the `supersaw` engine module as a PowerRP node widget.
 *
 * SOURCE family (warm amber header): it generates signal from nothing.
 *
 * Many detuned sawtooth voices at once — the wide, shimmering pad oscillator. Its width comes from the voices beating against each other, so Spread is the knob that matters.
 *
 ...
 * SUPERSAW_SPEC in core/audio_specs.js — its ports, its knobs and their ranges,
...
import { SUPERSAW_SPEC } from "../core/audio_specs.js";

export const audioSupersawPlugin = audioNodePlugin(SUPERSAW_SPEC);
