/**
 * OUTPUT — the `output` engine module as a PowerRP node widget.
 *
 * OUTPUT family (rose header): sound leaves here.
 *
 * Where sound leaves, through a limiter. MULTIPLE OUTPUTS SUM (user ruling, ADDENDUM 10: 'If we have multiple audio outputs by the way, we'll just add them all together') — N of these coexist and are never an error.
 *
 ...
 * OUTPUT_SPEC in core/audio_specs.js — its ports, its knobs and their ranges,
...
import { OUTPUT_SPEC } from "../core/audio_specs.js";

export const audioOutputPlugin = audioNodePlugin(OUTPUT_SPEC);
