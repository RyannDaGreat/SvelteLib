...
<script>
  ...
  const coreCommands = [
    ...
    // along per the orchestrator note; it is a real stored key on every transform
    // widget (defaults, e.g. plugins/rect.js) even though no plugin currently
    // exposes an Inspector row for it (core/properties.js BUNDLES.transform has no
    // "scale" entry) — so it captures and transports fine on the NO-SELECTION path
    // (item_properties_clipboard.js's per-id transport reads raw state, not rows),
    // but a SELECTION-targeted retarget always skips it: retargetedState requires
    // the TARGET plugin to declare a row for a key (core/item_properties_clipboard.js),
