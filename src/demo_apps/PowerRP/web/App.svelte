...
<script>
  ...
  const coreCommands = [
    ...
    // hover-to-preview trope the preset cards established.
    {
      ...
      // four rows under Transform — so the pair now reads
    ...
    // THE THREE SUBSETS, asked for in one breath with the whole-state one (user,
    ...
    // palette, and also, of course, tools. It's kind of like copy properties, but
    // for a limited subset."
    //
    // They are the SAME verb with a narrower capture — one `keys` argument, the
    // same payload kind, the same Paste. No new chord: Cmd+Shift+C stays the whole
    ...
    { id: "copy-dimensions", title: "Copy Dimensions (just width and height, to paste onto the SAME widget on another slide)", icon: "mdi:ruler-square", aliases: ["copy size", "copy width height", "copy wh", "copy dimensions", "copy w h"], when: needsSelection, requires: "at least one selected widget whose dimensions can be copied", help: "Copies width and height only — how big the widget is here, equations verbatim. Pasting keyframes ONLY those two properties onto the SAME widget on the current slide; it does not move it. A stored width or height may be NEGATIVE — that sign is how a flip is recorded — so copying dimensions from a flipped widget carries the flip with them. Still puts the picture on your system clipboard, exactly as Copy does.", run: (a) => a.copySelectionProperties(["w", "h"], "Copy Dimensions") },
    { id: "copy-box", title: "Copy Box (X, Y, width and height, to paste onto the SAME widget on another slide)", icon: "mdi:vector-square", aliases: ["copy x y h w", "copy bounds", "copy box", "copy rect", "copy frame", "copy position and size"], when: needsSelection, requires: "at least one selected widget whose box can be copied", help: "Copies the whole box — X, Y, width and height, equations verbatim — and nothing else. Pasting keyframes ONLY those four properties onto the SAME widget on the current slide, so it lands exactly where and how big it was, while its colour, text and effects stay put. Copy Position and Copy Dimensions are the two halves of this if you want only one of them. Still puts the picture on your system clipboard, exactly as Copy does.", run: (a) => a.copySelectionProperties(["x", "y", "w", "h"], "Copy Box") },
