 ...
 * frameCount) — the heavy exportVideo command needs a DOM (canvas averaging) and
 * an encoder, so it is browser-only, but its timing math is unit-testable in node.
 */
 ...
 *
 * Per output frame i: for each of `samples` sub-times t (subFrameTimes), it sets
 * the controlled time (`setTime(t)` — the caller wires this to the ambient
 * particle/shader clock so time-driven effects blur too), samples the timeline
 * (sampleTimeline → slide, alpha), renders (`renderFrame`), and — when
 * samples>1 — AVERAGES the sub-frames (box filter) before handing one frame to
 * the encoder at a monotonic 1/fps timestamp. samples=1 skips all averaging.
 *
 ...
 * @param {(fraction:number)=>void} [o.onProgress] 0..1 after each output frame.
 * @param {AbortSignal} [o.signal] Cancels the export (throws AbortError).
 * @returns {Promise<Blob>}
 */
export async function exportVideo({ plan, renderFrame, encoder, width, height, fps, samples = DEFAULT_SAMPLES, setTime = () => {}, onProgress, signal }) {
  ...
  // holds the divided result the encoder reads.
  ...
  try {
    for (let i = 0; i < total; i++) {
      ...
      const times = subFrameTimes(i, n, fps);
      let frameSource;
      if (n === 1) {
        frameSource = await renderAt(times[0]);
      } else {
        accum.fill(0);
    ...
    setTime(null); // release the controlled-time override no matter what
  }
