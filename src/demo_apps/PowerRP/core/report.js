/**
 * Log-once error reporting — ONE home for the "report loudly, but never spam"
 * pattern (previously copied as private Sets in core/expressions.js and
 * plugins/fancy_arrow.js). DOM-free pure JS.
 *
 * THROTTLE SEMANTICS (deliberate — documenting the Opus1 review finding #5
 * question): each unique key logs ONCE per process/page session. These
 * reports fire from per-frame / per-evaluation-pass code (emit(), the
 * expression pass), so re-keying per pass would console.error at display
 * rate; the trade-off is that a RECURRING error stays silent after its first
 * report until reload. The dedup set grows only with DISTINCT message texts
 * (bounded in practice by how many different errors a session can produce);
 * capping it would need a ratified constant — flagged, not invented.
 */
